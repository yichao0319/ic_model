...
%%
%% example:
%%  plot_sim_internal_link_v4_results([10000:10000:20000], 2, 8, 1, 1, 'cal')
%%  plot_sim_internal_link_v4_results([10000:10000:20000], 3, 10, 1, 1, 'cal')
%%  plot_sim_internal_link_v4_results([10000:10000:20000], 1, 1, 1, 1, 'cal')
%%  plot_sim_internal_link_v4_results([10000:10000:20000], 1, 100000, 1, 1, 'cal')
%%  plot_sim_internal_link_v4_results([10000:10000:20000], 3, 100000, 1, 1, 'cal')
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function plot_sim_internal_link_v4_results(Ns, L, U, lambda, eta, sel_type)
    ...
    for ni = 1:length(Ns)
        ...
        % pause
        data = load(sprintf('%sL%dU%dN%d.internal_link_v4.l%.2f.e%.2f.%s.txt', input_dir, L, U, N, lambda, eta, sel_type));
        idx = find(data(:,1) > 0);
        data = data(idx,:);
        x = data(:,1);
        y = data(:,2) / sum(data(:,2));
        ...
        lh = plot(x, y, 'ro');
        set(lh, 'Color', colors{mod(ni-1,length(colors))+1});
        set(lh, 'marker', markers{mod(ni-1,length(markers))+1});
        hold on;
    end
    ...
    title(sprintf('L=%d,U=%d,lambda=%.2f,eta=%.2f', L, U, lambda, eta));
...
function plot_sim_internal_link_one_result(N, L, U, itvl, sel_type)
    ...
    data = load(sprintf('%s%s.internal_link_v4.l%.2f.e%.2f.%s.txt', input_dir, filename, lambda, eta, sel_type));
